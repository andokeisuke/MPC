...
int main(int argc, char **argv){
  ...
  ros::NodeHandle nh;
  nh.getParam("str_sim_node/initial_x", init_pose[0]);
  nh.getParam("str_sim_node/initial_y", init_pose[1]);
  nh.getParam("str_sim_node/initial_th", init_pose[2]);

  for(int i=0;i<3;i++){

      position[i] = init_pose[i];
